...
(() => {
  ...
  function shootProjectile(context, monster, target, angleOffset = 0, options = {}) {
    ...
    const solarEnemy = monster.type === "sunLancer" || monster.type === "mirageCaster" || monster.type === "solarRunner" || monster.type === "prismBeacon" || monster.type === "solarWarden" || monster.type === "sunspireKeeper" || monster.type === "emberDragon";
    const midbossColor = monster.type === "solarWarden" ? "#fff0a6" : monster.type === "towerWarden" ? "#d9f7ff" : monster.type === "frostGolem" ? "#8dd7ff" : monster.type === "warden" ? "#6de4ff" : monster.type === "mistKeeper" ? "#9fd6c7" : monster.type === "cryptWarden" ? "#d7b26d" : monster.type === "archiveWarden" ? "#b08cff" : monster.type === "obsidianGolem" ? "#aab0c8" : "#55c7a0";
    ...
    state.projectiles = state.projectiles.filter((p) => {
      ...
      if (rectsOverlap(player, hitbox)) {
        if (p.hitCooldown <= 0 && player.invuln <= 0 && player.hp > 0) {
          ...
          if (p.source === "wisp" || p.source === "dragon") {
          ...
          } else if (p.source === "sunLancer" || p.source === "mirageCaster" || p.source === "solarRunner" || p.source === "prismBeacon" || p.source === "solarWarden" || p.source === "sunspireKeeper" || p.source === "emberDragon") {
            ...
            const baseSlow = p.source === "emberDragon" ? 1700 : p.source === "solarWarden" || p.source === "sunspireKeeper" ? 1350 : 950;
            const baseStamina = p.source === "emberDragon" ? 30 : p.source === "solarWarden" || p.source === "sunspireKeeper" ? 24 : 16;
