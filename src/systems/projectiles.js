...
(() => {
  ...
  function shootProjectile(context, monster, target, angleOffset = 0, options = {}) {
    ...
    const solarEnemy = monster.type === "sunLancer" || monster.type === "mirageCaster" || monster.type === "solarWarden" || monster.type === "emberDragon";
    const midbossColor = monster.type === "solarWarden" ? "#fff0a6" : monster.type === "towerWarden" ? "#d9f7ff" : monster.type === "frostGolem" ? "#8dd7ff" : monster.type === "warden" ? "#6de4ff" : monster.type === "mistKeeper" ? "#9fd6c7" : monster.type === "cryptWarden" ? "#d7b26d" : monster.type === "obsidianGolem" ? "#aab0c8" : "#55c7a0";
    const projectileColor = solarEnemy ? (monster.type === "mirageCaster" ? "#ff9f5a" : "#fff0a6") : monster.type === "frostDragon" || monster.type === "frostMoth" || monster.type === "frostGolem" || monster.type === "towerWarden" ? "#b9f4ff" : monster.type === "bubbler" ? "#8dd7ff" : monster.type === "voidDragon" ? "#d8d8ff" : monster.type === "voidWraith" ? "#7b80d8" : monster.type === "obsidianCrawler" || monster.type === "obsidianGolem" ? "#aab0c8" : monster.type === "eclipseDragon" ? "#f06dff" : monster.type === "eclipseMage" ? "#e36dff" : monster.type === "summoner" ? "#d678ff" : monster.type === "moonShade" ? "#7f8cff" : monster.type === "sorcerer" ? "#b990ff" : monster.boss ? "#ff543d" : monster.midboss ? midbossColor : "#ffd166";
    ...
    state.projectiles.push({
      ...
      life: options.life || (monster.type === "emberDragon" ? 3000 : monster.type === "solarWarden" || monster.type === "sunLancer" ? 2600 : monster.type === "mirageCaster" ? 2200 : monster.type === "frostDragon" ? 1850 : monster.type === "voidDragon" ? 1750 : monster.type === "eclipseDragon" ? 1650 : monster.boss ? 1500 : monster.midboss ? 1350 : 1200),
    ...
    state.projectiles = state.projectiles.filter((p) => {
      ...
      if (rectsOverlap(player, hitbox)) {
        if (p.hitCooldown <= 0 && player.invuln <= 0 && player.hp > 0) {
          const source = p.source === "sunLancer" || p.source === "mirageCaster" || p.source === "solarWarden" || p.source === "emberDragon" ? "solar" : p.source === "frostDragon" || p.source === "frostGolem" || p.source === "frostMoth" || p.source === "towerWarden" ? "frost" : p.source === "obsidianGolem" || p.source === "obsidianCrawler" ? "obsidian" : p.source === "voidDragon" || p.source === "voidWraith" ? "void" : p.source === "eclipseDragon" || p.source === "eclipseMage" ? "eclipse" : p.source === "wisp" || p.source === "dragon" ? "fire" : p.source === "bubbler" ? "bubble" : p.source === "sorcerer" || p.source === "summoner" || p.source === "moonShade" || p.source === "ashKnight" || p.source === "mistKeeper" || p.source === "cryptWarden" ? "magic" : "projectile";
          let hurt = Math.max(1, Math.round((p.damage - Math.floor(playerDefense() * 0.45)) * armorDamageMultiplier({ boss: p.source === "dragon" || p.source === "eclipseDragon" || p.source === "voidDragon" || p.source === "frostDragon" || p.source === "emberDragon", midboss: p.source === "ashKnight" || p.source === "mistKeeper" || p.source === "cryptWarden" || p.source === "frostGolem" || p.source === "solarWarden", type: p.source }, 0, source)));
          ...
          } else if (p.source === "sunLancer" || p.source === "mirageCaster" || p.source === "solarWarden" || p.source === "emberDragon") {
            const solarGuard = player.armor === 13 || activeAccessory(player, "horizon", "horizonCharm") || player.shield === 7;
            const baseSlow = p.source === "emberDragon" ? 1700 : p.source === "solarWarden" ? 1350 : 950;
            const baseStamina = p.source === "emberDragon" ? 30 : p.source === "solarWarden" ? 24 : 16;
